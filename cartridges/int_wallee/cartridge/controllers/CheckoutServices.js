...
server.prepend("SubmitPayment", function (req, res, next) {
    ...
    if (TransactionHelper.getIntegrationType() == "iframe" &&
        (empty(req.form.WALLEE_handler) ||
            (typeof req.form.WALLEE_handler !== 'string') ||
            (req.form.WALLEE_handler.match(paymentMethodRegex) === null))) {
        res.json({
            handler: {
                handler: req.form.WALLEE_handler,
                regex: req.form.WALLEE_handler ? req.form.WALLEE_handler.match(paymentMethodRegex) : null
            },
            form: billingForm,
            fieldErrors: [],
            serverErrors: [],
    ...
    if (dw.system.HookMgr.hasHook("app.payment.processor." + processor.ID.toLowerCase())) {
        ...
        throw new Error("hooks/payment/processor/wallee.js file is missing or \"app.payment.processor.wallee\" hook is wrong set");
    }
    ...
    basketModel.billing.payment.selectedPaymentInstruments[0].expirationMonth = req.form.WALLEE_handler_name + " ";
