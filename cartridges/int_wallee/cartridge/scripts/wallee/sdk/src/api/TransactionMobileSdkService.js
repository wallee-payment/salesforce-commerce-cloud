    ...
    TransactionMobileSdkService.prototype.paymentFormUrl = function (credentials, options) {
        ...
        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
            ...
            try {
                ...
                dw.system.Logger.error("wallee > TransactionMobileSdkService > paymentFormUrl > unable to parse JSON > " + JSON.stringify({ message: e.message, fileName: e.fileName, lineNumber: e.lineNumber }));
            }
            return body;
        }
        else {
            ...
            throw new Error("wallee > TransactionMobileSdkService > paymentFormUrl > " + error.type + " > " + JSON.stringify(error));
        }
    };
