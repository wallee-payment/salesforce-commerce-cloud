    ...
    TransactionPaymentPageService.prototype.paymentPageUrl = function (spaceId, id, options) {
        ...
        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
            ...
            try {
                ...
                dw.system.Logger.error("wallee > TransactionPaymentPageService > paymentPageUrl > unable to parse JSON > " + JSON.stringify({ message: e.message, fileName: e.fileName, lineNumber: e.lineNumber }));
            }
            return body;
        }
        else {
            ...
            throw new Error("wallee > TransactionPaymentPageService > paymentPageUrl > " + error.type + " > " + JSON.stringify(error));
        }
    };
